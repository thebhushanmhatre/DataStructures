...
// in preorder, without creating a new linked list
...
console.log('Orginial tree is', dummytree);
flatten(dummytree, prev);

// // To solve without global variable: Important

// function flatten(node) {
//   if (!node) return null;

//   let rightPrev = flatten(node.right);
//   let leftPrev = flatten(node.left);

//   // Connect: current -> right subtree (or left subtree if no right) -> prev chain
//   node.right = rightPrev || leftPrev;
//   node.left = null;
...
console.log('Flattened list is', JSON.stringify(dummytree));
