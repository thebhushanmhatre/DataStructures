...
// works for both zeroes and negatives
let nums = [1, 2, 3, 1, 0, 0, 1, 1, 1];

function findLongestSubArr(nums, k) {
  let sumMap = {};
  let maxLen = 0;
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    let remainder = sum - k;
    if (remainder && sumMap[remainder]) {
      let len = i - sumMap[remainder];
      maxLen = Math.max(maxLen, len);
    }
  }

  console.log('Longest Subarry length is', maxLen);
}

findLongestSubArr(nums, 3);
