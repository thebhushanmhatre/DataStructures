...
var combinationSum2 = function (candidates, target) {
  ...
  [...uniqueStrs].forEach((str) =>
    result.push(str.split('').map((i) => Number(i)))
  );
  return result;
};
...
//
//
function combinationSum(nums, sum, ans, index = 0, ds = []) {
  if (sum < 0) {
    return;
  }
  ...
  combinationSum(nums, sum - nums[index], ans, index + 1, [...ds, nums[index]]);

  // removing the next duplicate element in case not selected
  while (nums[index] == nums[index + 1]) {
    index++;
  }
  // not selecting the current element
  combinationSum(nums, sum, ans, index + 1, ds);
}
